...
static struct option long_options[] = {
    ...
    { "asserver",      no_argument,       NULL, ASSERVER_OPTION },
	{ 0, 0, 0, 0 }
};

void signal_handler(int sig) { exit_main(); exit(1); }
		...
		switch (opt)
		{
			...
			case LOG_OPTION:
			{
				...
				break;
                        ...
                        case ASSERVER_OPTION:
                        {
                                settings.as_server = 1;
                                break;
                        }
...
int use_tunnel(void)
{
	...
	static struct wrap_pkt wp;
	long dbufl;
	long bs;
	//printf("%d\n",sizeof(struct wrap_pkt)-BUFSIZE);
	for (;;)
	{
		FD_ZERO(&io);
